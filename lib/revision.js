...
function run() {
  ...
  hexo.tmpCache = hexo.tmpCache || Object.create(null);
  ...
  return Promise.mapSeries(routes, function(path){
    ...
    return streamToArrayAsync(stream)
      .then(function(arr) {
            ...
            if(attr){
              htmlObj.setAttribute(selector, attr, function(oldValue){
                if(isLocalPath(oldValue)){
                  ...
                  const newValue = hexo.tmpCache[oldValue];
                  fileUpdated = true;
                  return '/'+newValue;
                }else{
                  return oldValue;
        ...
        if(ext.match(/(\.js|\.css)/)){
          for(let key in hexo.tmpCache){
            if(buffer.match(key)){
              buffer = buffer.replace(key, hexo.tmpCache[key]);
            }
          }
        }
        ...
        hexo.tmpCache[path] = revisedPath;
