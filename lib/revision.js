  ...
  return Promise.mapSeries(routes, function(path){
    ...
    if(ext.match(/(\.js|\.css)/)){
      for(let key in revIndex){
        if(bufferAsString.match(root + key)){
          buffer = Buffer.from(bufferAsString.replace(key, revIndex[key]));
          // console.log('[revision] ' + key + ' >>>>>>>>>>>>>>>>>> ' + revIndex[key]);
        } else if (bufferAsString.match(getFileName(key))) {
          // Match and replace filename instead of path
          buffer = Buffer.from(bufferAsString.replace(getFileName(key), getFileName(revIndex[key])));
